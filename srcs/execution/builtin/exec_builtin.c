...
/*   Updated: 2023/05/15 20:43:38 by mdorr            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	child_job_builtin(t_command *command, const t_builtin_mapper *map)
{
...
void	exec_builtin(t_command *command)
{
	...
	pid = fork();
	if (pid == 0)
		child_job_builtin(command, map);
	else if (pid > 0)
	{
		close_pipes_parent(command);
		wait(NULL);
